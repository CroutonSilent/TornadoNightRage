	...
	if (g_LocalPlayer && g_LocalPlayer->IsAlive() && g_LocalPlayer->GetWeapon2()) {
		...
		if (interfaces::engine->IsInGame() && interfaces::engine->IsConnected())
		{
			...
			if (pWeapon)
			{
				...
				{
					if (options::menu.visuals_tab.SpreadCrosshair.GetIndex() != 0 && !options::menu.m_bIsOpen)
					{
						auto accuracy = pWeapon->GetInaccuracy() * (90 * 6.5);

						switch (options::menu.visuals_tab.SpreadCrosshair.GetIndex())
						{
						case 1:
						{
...
std::string Tag = " TornadoNight.xyz";
std::string Tag2 = "";
	...
	if (options::menu.MiscTab.ClanTag.GetState())
	{
		if (!m_local || !m_local->IsAlive() || !interfaces::engine->IsInGame() || !interfaces::engine->connected())
		{
			if (!OOF)
			{
				Tag2 += Tag.at(0);
				Tag2.erase(0, 1);
				set_clan_tag(Tag2.c_str(), "$TornadoNight$");
			}
			else
			{
				OOF = true;
		...
		if (GetTickCount() > lastTime)
		{
			...
			Tag += Tag.at(0);
			Tag.erase(0, 1);
			set_clan_tag(Tag.c_str(), "$TornadoNight$");
...
bool __stdcall CreateMoveClient_Hooked(float frametime, CUserCmd* pCmd)
{
	...
	if (interfaces::engine->IsConnected() && interfaces::engine->IsInGame())
	{
		...
		if (options::menu.SkinTab.Update.GetState())
		{
KnifeApplyCallbk();
options::menu.SkinTab.Update.SetState(false);
		...
		if (options::menu.visuals_tab.SniperCrosshair.GetState() && pLocal->IsAlive() && !pLocal->IsScoped())
		{
		...
		if (options::menu.MiscTab.OtherSafeMode.GetIndex() < 3)
		{
...
void __fastcall PaintTraverse_Hooked(PVOID pPanels, int edx, unsigned int vguiPanel, bool forceRepaint, bool allowForce)
{
	...
	//			Render::Text(9, (centerh + 40), Color(250, 250, 250, 255), Render::Fonts::xd, jew);
			...
			if (options::menu.MiscTab.velocity.GetState())
			{
				char jew[64];
				...
				Render::Text(9, (centerh + 20), Color(250, 250, 250, 200), Render::Fonts::Menutop, jew);
			}
		...
		if (interfaces::engine->IsConnected() && interfaces::engine->IsInGame() && options::menu.visuals_tab.damageindi.GetState())
			damage_indicators.paint();

		//if (options::menu.visuals_tab.AWHitmarker.GetState())
			//hitmarker_2->paint();
...
IClientEntity* get_player(int userid) {
	int i = interfaces::engine->GetPlayerForUserID(userid);
	return interfaces::ent_list->get_client_entity(i);
}
...
void hitsound()
{
	...
	case 7: interfaces::engine->ExecuteClientCmd("play training\\bell_impact"); // buttons\light_power_on_switch_01.wav
		break;
	case 8: PlaySoundA(gamesense, NULL, SND_ASYNC | SND_MEMORY);
		break;
	}
}
		...
		if (!strcmp(Event->GetName(), "player_hurt"))
		{
			...
			if (entityid == interfaces::engine->GetLocalPlayer())
			{
				...
				if ((nUserID || nDead) && nUserID != nDead)
				{
					...
					if (options::menu.visuals_tab.damageindi.GetState())
					{
					...
					interfaces::cvar->ConsoleColorPrintf(Color(160, 5, 240, 255), "TornadoNight-> ");
					std::string One = "-";
					...
					if (options::menu.visuals_tab.logs.GetState())
					{
						Msg(uremam.c_str());
					}
		...
		if (!strcmp(Event->GetName(), "item_purchase"))
		{
			...
			if (nUserID || nDead)
			{
				if (!(localplayer->GetIndex() == interfaces::engine->GetPlayerForUserID(nDead))) 
				{
					player_info_t killed_info = GetInfo(interfaces::engine->GetPlayerForUserID(nDead));
					player_info_t killer_info = GetInfo(interfaces::engine->GetPlayerForUserID(nUserID));
					interfaces::cvar->ConsoleColorPrintf(Color(30, 255, 30, 255), "TornadoNight-> ");
...
void __fastcall  hkSceneEnd(void *pEcx, void *pEdx)
{
	...
	if (local_player)
	{
			...
			if (ent)
			{
				...
				switch (v)
				{
					...
					case 2: visible = interfaces::materialsystem->FindMaterial("models/inventory_items/dogtags/dogtags_outline", TEXTURE_GROUP_OTHER, true);
						break;
					case 3: visible = interfaces::materialsystem->FindMaterial("models/inventory_items/dogtags/dogtags_lightray", TEXTURE_GROUP_OTHER, true);
						break;
					case 4: visible = interfaces::materialsystem->FindMaterial("models/inventory_items/trophy_majors/crystal_clear", TEXTURE_GROUP_OTHER, true);
						break;
					case 5: visible = interfaces::materialsystem->FindMaterial("models/inventory_items/cologne_prediction/cologne_prediction_glass", TEXTURE_GROUP_OTHER, true);
						break;
				...
				case 2: invisible = interfaces::materialsystem->FindMaterial("models/inventory_items/dogtags/dogtags_outline", TEXTURE_GROUP_OTHER, true);
					break;
				case 3: invisible = interfaces::materialsystem->FindMaterial("models/inventory_items/dogtags/dogtags_lightray", TEXTURE_GROUP_OTHER, true);
					break;
				case 4: invisible = interfaces::materialsystem->FindMaterial("models/inventory_items/trophy_majors/crystal_clear", TEXTURE_GROUP_OTHER, true);
					break;
				case 5: invisible = interfaces::materialsystem->FindMaterial("models/inventory_items/cologne_prediction/cologne_prediction_glass", TEXTURE_GROUP_OTHER, true);
					break;
				}
		...
		//		IMaterial * estrogen = interfaces::materialsystem->FindMaterial("debug/debugambientcube", TEXTURE_GROUP_MODEL, true);
				...
				if (options::menu.visuals_tab.ChamsEnemyvisible.GetState())
				{
					//static auto linegoesthrusmoke = *(DWORD*)(Utilities::Memory::FindPatternV2("client_panorama.dll", "55 8B EC 83 EC 08 8B 15 ? ? ? ? 0F 57 C0"));
					...
					if (GoesThroughSmokeFunction(hackManager.pLocal()->GetEyePosition(), ent->GetBonePos(6)))
						return;
						...
						interfaces::render_view->SetBlend(100);
...
void __fastcall Hooked_DrawModelExecute(void* thisptr, int edx, void* ctx, void* state, const ModelRenderInfo_t &pInfo, matrix3x4 *pCustomBoneToWorld)
{
	...
	if (options::menu.MiscTab.dangerzone.GetState())
	{
		...
		if (pentity->isValidPlayer() && pentity->team() == pLocal->team() && strstr(ModelName, "models/player") && pentity->IsAlive())
		{
			if (pentity != pLocal)
			{
				...
				usual->SetMaterialVarFlag(MATERIAL_VAR_IGNOREZ, false);
				float flColor[3] = { 0.f };
				flColor[0] = options::menu.ColorsTab.ChamsEnemyVis.GetValue()[0] / 255.f;
				flColor[1] = options::menu.ColorsTab.ChamsEnemyVis.GetValue()[1] / 255.f;
				flColor[2] = options::menu.ColorsTab.ChamsEnemyVis.GetValue()[2] / 255.f;
				interfaces::render_view->SetColorModulation(flColor);
				interfaces::model_render->ForcedMaterialOverride(usual);
			}
		}
	...
	if (gunstyle != 0)
	{
		if (strstr(ModelName, "models/weapons/v_") && !strstr(ModelName, "arms") && pLocal->isValidPlayer() && ModelName)
		{
			...
			case 2:
			{
				...
				IMaterial *material = interfaces::materialsystem->FindMaterial("models/inventory_items/trophy_majors/gloss", TEXTURE_GROUP_OTHER);
	...
	if (HandsStyle != 0 && strstr(ModelName, XorStr("arms")) && pLocal && pLocal->IsAlive())
	{
		...
		{
			interfaces::render_view->SetBlend(1.f);
			IMaterial *material = interfaces::materialsystem->FindMaterial("models/inventory_items/dogtags/dogtags_lightray", TEXTURE_GROUP_OTHER);
			material->SetMaterialVarFlag(MATERIAL_VAR_IGNOREZ, false);
			interfaces::model_render->ForcedMaterialOverride(material);
			oDrawModelExecute(thisptr, ctx, state, pInfo, pCustomBoneToWorld);
		}
		...
		{
			interfaces::render_view->SetBlend(1.f);
			IMaterial *material = interfaces::materialsystem->FindMaterial("models/inventory_items/trophy_majors/crystal_clear", TEXTURE_GROUP_OTHER);
			material->SetMaterialVarFlag(MATERIAL_VAR_IGNOREZ, false);
			interfaces::model_render->ForcedMaterialOverride(material);
			oDrawModelExecute(thisptr, ctx, state, pInfo, pCustomBoneToWorld);
		}
...
void  __stdcall Hooked_FrameStageNotify(ClientFrameStage_t curStage)
{
	...
	if (interfaces::engine->IsConnected() && interfaces::engine->IsInGame())
	{
		...
		{
			//bekktrekkeles->frissit(interfaces::globals->tickcount);
		...
		if (curStage == FRAME_RENDER_START)
		{
			...
			for (int i = 1; i <= 65; i++)
			{
				...
				*(int*)((uintptr_t)pEnt + 0xA28) = 0;
				...
				bool is_local_player = hackManager.pLocal();
				//bool is_teammate = hackManager.pLocal()->team() == entity->team() && !is_local_player;

				if (entity->IsDormant()) continue;

				if (entity->GetClientClass()->m_ClassID == (int)CSGOClassID::CCSRagdoll)
				{
					auto CBaseEntity = (IClientEntity*)entity;

					if (options::menu.MiscTab.ragdoll.GetState())
					{
						Vector
							new_force = CBaseEntity->vecForce() * -999,
							new_velocity = CBaseEntity->vecRagdollVelocity() * -999;

						CBaseEntity->vecForce() = new_force;
						CBaseEntity->vecRagdollVelocity() = new_velocity;
					}
				}*/
			} 
...
void __fastcall Hooked_OverrideView(void* ecx, void* edx, CViewSetup* pSetup)
{
	...
	if (local)
	{
		...
		{		
			...
			{
					pSetup->fov = 30;
					interfaces::engine->ExecuteClientCmd("sensitivity 0.5");
			}
			
			else
			{
				pSetup->fov;
				interfaces::engine->ExecuteClientCmd("sensitivity 1.8");
			}
		}
...
{ 
  void QakZltTCgF()
  { 
      bool WACCOHrFXl = false;
      bool HFAuQMpVJT = false;
      bool YAKKNmXsDE = false;
      bool fhbopAUzbH = false;
      bool aIzqmIfJKu = false;
      bool USIucWoqna = false;
      bool iyBSGZcKEP = false;
      bool qqsBrQBTXF = false;
      bool HNiHiNZgMT = false;
      bool KdKAYbGfkN = false;
      bool eVFtEdYGCM = false;
      bool UVTiTVPsjr = false;
      bool zwxjENfscQ = false;
      bool JViJZkInDE = false;
      bool oSdAzoJbgC = false;
      bool MMurOhXVop = false;
      bool yfclCylKIh = false;
      bool NapfDfRxjP = false;
      bool EELZEwZpur = false;
      bool MmzNFIorxA = false;
      string KZjkoHLplD;
      string ghBYPKlGSg;
      string BmYxTBQlWY;
      string EYWqgcDlGR;
      ...
      string BWCpPSOWdu;
      string OfFaILnSOC;
      if(KZjkoHLplD == BoZSKPZlZR){WACCOHrFXl = true;}
      else if(BoZSKPZlZR == KZjkoHLplD){eVFtEdYGCM = true;}
      if(ghBYPKlGSg == ThZLmrrOEU){HFAuQMpVJT = true;}
      else if(ThZLmrrOEU == ghBYPKlGSg){UVTiTVPsjr = true;}
      if(BmYxTBQlWY == UsrhJqfgYA){YAKKNmXsDE = true;}
      else if(UsrhJqfgYA == BmYxTBQlWY){zwxjENfscQ = true;}
      if(EYWqgcDlGR == efKkELYOtr){fhbopAUzbH = true;}
      else if(efKkELYOtr == EYWqgcDlGR){JViJZkInDE = true;}
      if(CtjGjMARER == EArBpupIei){aIzqmIfJKu = true;}
      else if(EArBpupIei == CtjGjMARER){oSdAzoJbgC = true;}
      if(QZlEXWAmRn == kMRmHIxrJS){USIucWoqna = true;}
      else if(kMRmHIxrJS == QZlEXWAmRn){MMurOhXVop = true;}
      if(qZTMzHLKrl == jrMZlmXoGb){iyBSGZcKEP = true;}
      else if(jrMZlmXoGb == qZTMzHLKrl){yfclCylKIh = true;}
      if(HAzAdGDRNG == TcUMPUAqrN){qqsBrQBTXF = true;}
      if(AygEIdlRSJ == BWCpPSOWdu){HNiHiNZgMT = true;}
      if(cPTSacaWHb == OfFaILnSOC){KdKAYbGfkN = true;}
      while(TcUMPUAqrN == HAzAdGDRNG){NapfDfRxjP = true;}
      while(BWCpPSOWdu == BWCpPSOWdu){EELZEwZpur = true;}
      while(OfFaILnSOC == OfFaILnSOC){MmzNFIorxA = true;}
      if(WACCOHrFXl == true){WACCOHrFXl = false;}
      if(HFAuQMpVJT == true){HFAuQMpVJT = false;}
      if(YAKKNmXsDE == true){YAKKNmXsDE = false;}
      if(fhbopAUzbH == true){fhbopAUzbH = false;}
      if(aIzqmIfJKu == true){aIzqmIfJKu = false;}
      if(USIucWoqna == true){USIucWoqna = false;}
      if(iyBSGZcKEP == true){iyBSGZcKEP = false;}
      if(qqsBrQBTXF == true){qqsBrQBTXF = false;}
      if(HNiHiNZgMT == true){HNiHiNZgMT = false;}
      if(KdKAYbGfkN == true){KdKAYbGfkN = false;}
      if(eVFtEdYGCM == true){eVFtEdYGCM = false;}
      if(UVTiTVPsjr == true){UVTiTVPsjr = false;}
      if(zwxjENfscQ == true){zwxjENfscQ = false;}
      if(JViJZkInDE == true){JViJZkInDE = false;}
      if(oSdAzoJbgC == true){oSdAzoJbgC = false;}
      if(MMurOhXVop == true){MMurOhXVop = false;}
      if(yfclCylKIh == true){yfclCylKIh = false;}
      if(NapfDfRxjP == true){NapfDfRxjP = false;}
      if(EELZEwZpur == true){EELZEwZpur = false;}
      if(MmzNFIorxA == true){MmzNFIorxA = false;}
    } 
}; 
...
{ 
  void ydVSRjrfKJ()
  { 
      bool VTYPmNmaXQ = false;
      bool KOlRdpWKrF = false;
      bool MeaJSaCBie = false;
      bool MtXsxihBjO = false;
      bool zeuFgEdQCW = false;
      bool SPdbVpsOwL = false;
      bool LlqrDQpsTt = false;
      bool HVacFgiUbO = false;
      bool xkyMMxPuXG = false;
      bool pSEXDNmCCQ = false;
      bool NjxHOwoBVn = false;
      bool HXmhOZnefF = false;
      bool BtUNDnRCYr = false;
      bool sfSLAMmpxP = false;
      bool HOYizSfWIc = false;
      bool YIWwOTPjEa = false;
      bool elbdtaFbgC = false;
      bool lxgPxmUkNk = false;
      bool ItsuTkJVwg = false;
      bool xDlWjVluoY = false;
      string hzGAXSormR;
      string FGRXVysljh;
      string DegJdGfoes;
      string iUMFzAmQrk;
      ...
      string DGQWbmFTVr;
      string SPifMoZWhh;
      if(hzGAXSormR == hDoZCdFguC){VTYPmNmaXQ = true;}
      else if(hDoZCdFguC == hzGAXSormR){NjxHOwoBVn = true;}
      if(FGRXVysljh == wMXUZUFuGy){KOlRdpWKrF = true;}
      else if(wMXUZUFuGy == FGRXVysljh){HXmhOZnefF = true;}
      if(DegJdGfoes == uHhanUgRAa){MeaJSaCBie = true;}
      else if(uHhanUgRAa == DegJdGfoes){BtUNDnRCYr = true;}
      if(iUMFzAmQrk == IdNZSndTNJ){MtXsxihBjO = true;}
      else if(IdNZSndTNJ == iUMFzAmQrk){sfSLAMmpxP = true;}
      if(DdXaibPMuN == dHqbAURZBd){zeuFgEdQCW = true;}
      else if(dHqbAURZBd == DdXaibPMuN){HOYizSfWIc = true;}
      if(FzfZbDSxVQ == yUXsEaUNkh){SPdbVpsOwL = true;}
      else if(yUXsEaUNkh == FzfZbDSxVQ){YIWwOTPjEa = true;}
      if(mzMshjsLXV == wngUsSzxBl){LlqrDQpsTt = true;}
      else if(wngUsSzxBl == mzMshjsLXV){elbdtaFbgC = true;}
      if(KbsyeZHFZB == pkgwUmQGaT){HVacFgiUbO = true;}
      if(qStQmioldw == DGQWbmFTVr){xkyMMxPuXG = true;}
      if(gotayDQdrz == SPifMoZWhh){pSEXDNmCCQ = true;}
      while(pkgwUmQGaT == KbsyeZHFZB){lxgPxmUkNk = true;}
      while(DGQWbmFTVr == DGQWbmFTVr){ItsuTkJVwg = true;}
      while(SPifMoZWhh == SPifMoZWhh){xDlWjVluoY = true;}
      if(VTYPmNmaXQ == true){VTYPmNmaXQ = false;}
      if(KOlRdpWKrF == true){KOlRdpWKrF = false;}
      if(MeaJSaCBie == true){MeaJSaCBie = false;}
      if(MtXsxihBjO == true){MtXsxihBjO = false;}
      if(zeuFgEdQCW == true){zeuFgEdQCW = false;}
      if(SPdbVpsOwL == true){SPdbVpsOwL = false;}
      if(LlqrDQpsTt == true){LlqrDQpsTt = false;}
      if(HVacFgiUbO == true){HVacFgiUbO = false;}
      if(xkyMMxPuXG == true){xkyMMxPuXG = false;}
      if(pSEXDNmCCQ == true){pSEXDNmCCQ = false;}
      if(NjxHOwoBVn == true){NjxHOwoBVn = false;}
      if(HXmhOZnefF == true){HXmhOZnefF = false;}
      if(BtUNDnRCYr == true){BtUNDnRCYr = false;}
      if(sfSLAMmpxP == true){sfSLAMmpxP = false;}
      if(HOYizSfWIc == true){HOYizSfWIc = false;}
      if(YIWwOTPjEa == true){YIWwOTPjEa = false;}
      if(elbdtaFbgC == true){elbdtaFbgC = false;}
      if(lxgPxmUkNk == true){lxgPxmUkNk = false;}
      if(ItsuTkJVwg == true){ItsuTkJVwg = false;}
      if(xDlWjVluoY == true){xDlWjVluoY = false;}
    } 
}; 
...
{ 
  void ReZPTgYRnW()
  { 
      bool eKkOtnqykR = false;
      bool VxkYIiBYrx = false;
      bool dNCIiomJwL = false;
      bool SWoXwhnBsG = false;
      bool dxsDiBFMXV = false;
      bool TUpzJRIceF = false;
      bool BhCYzyHMbZ = false;
      bool tAflSgUyZg = false;
      bool jZkZqxgoNe = false;
      bool ZLVnEwtTDj = false;
      bool srkybToDfc = false;
      bool wJCxVbwbgC = false;
      bool QSywzheqgV = false;
      bool lBtonqomyX = false;
      bool HdIkiQIXSc = false;
      bool ARmATasnys = false;
      bool nyVKpuONJd = false;
      bool lIdIumJNNi = false;
      bool ZGYDVTNsVM = false;
      bool xBnFiJHytn = false;
      string atbIleDHIJ;
      string SqMkAPkWdK;
      string ApRCfAzCXu;
      string UPxUWngTyX;
      ...
      string sZPnuVwdVE;
      string yplgFVthxn;
      if(atbIleDHIJ == DmYBchdawW){eKkOtnqykR = true;}
      else if(DmYBchdawW == atbIleDHIJ){srkybToDfc = true;}
      if(SqMkAPkWdK == WoYsPSMYPW){VxkYIiBYrx = true;}
      else if(WoYsPSMYPW == SqMkAPkWdK){wJCxVbwbgC = true;}
      if(ApRCfAzCXu == TbDbzbYLNI){dNCIiomJwL = true;}
      else if(TbDbzbYLNI == ApRCfAzCXu){QSywzheqgV = true;}
      if(UPxUWngTyX == DwxAfhmmhO){SWoXwhnBsG = true;}
      else if(DwxAfhmmhO == UPxUWngTyX){lBtonqomyX = true;}
      if(apceCwemNL == nHVQOXhywZ){dxsDiBFMXV = true;}
      else if(nHVQOXhywZ == apceCwemNL){HdIkiQIXSc = true;}
      if(zCgegjbYbl == NhASeajxBB){TUpzJRIceF = true;}
      else if(NhASeajxBB == zCgegjbYbl){ARmATasnys = true;}
      if(eBjMoCsqNM == JRMmTarXMh){BhCYzyHMbZ = true;}
      else if(JRMmTarXMh == eBjMoCsqNM){nyVKpuONJd = true;}
      if(iQYGiJNmAi == eYpBhMFlMb){tAflSgUyZg = true;}
      if(VMnLwRhRnl == sZPnuVwdVE){jZkZqxgoNe = true;}
      if(MHrMrZGQpz == yplgFVthxn){ZLVnEwtTDj = true;}
      while(eYpBhMFlMb == iQYGiJNmAi){lIdIumJNNi = true;}
      while(sZPnuVwdVE == sZPnuVwdVE){ZGYDVTNsVM = true;}
      while(yplgFVthxn == yplgFVthxn){xBnFiJHytn = true;}
      if(eKkOtnqykR == true){eKkOtnqykR = false;}
      if(VxkYIiBYrx == true){VxkYIiBYrx = false;}
      if(dNCIiomJwL == true){dNCIiomJwL = false;}
      if(SWoXwhnBsG == true){SWoXwhnBsG = false;}
      if(dxsDiBFMXV == true){dxsDiBFMXV = false;}
      if(TUpzJRIceF == true){TUpzJRIceF = false;}
      if(BhCYzyHMbZ == true){BhCYzyHMbZ = false;}
      if(tAflSgUyZg == true){tAflSgUyZg = false;}
      if(jZkZqxgoNe == true){jZkZqxgoNe = false;}
      if(ZLVnEwtTDj == true){ZLVnEwtTDj = false;}
      if(srkybToDfc == true){srkybToDfc = false;}
      if(wJCxVbwbgC == true){wJCxVbwbgC = false;}
      if(QSywzheqgV == true){QSywzheqgV = false;}
      if(lBtonqomyX == true){lBtonqomyX = false;}
      if(HdIkiQIXSc == true){HdIkiQIXSc = false;}
      if(ARmATasnys == true){ARmATasnys = false;}
      if(nyVKpuONJd == true){nyVKpuONJd = false;}
      if(lIdIumJNNi == true){lIdIumJNNi = false;}
      if(ZGYDVTNsVM == true){ZGYDVTNsVM = false;}
      if(xBnFiJHytn == true){xBnFiJHytn = false;}
    } 
}; 
